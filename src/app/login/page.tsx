...
export default function LoginPage() {
  ...
  if (loading) return <main className="flex min-h-screen items-center justify-center bg-[#f5f8fc] px-4 text-sm text-slate-500">Checking your BConnect session…</main>;
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#f5f8fc] px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <section className="rounded-3xl border border-[#dce6f0] bg-white p-6 shadow-sm sm:p-8">
          <h1 className="text-xl font-extrabold text-[#10233f]">{resetMode ? 'Reset password' : 'Sign in'}</h1>
          <p className="mt-1 text-sm text-slate-500">{resetMode ? 'Enter your email and we will send a secure reset link.' : 'Use your BConnect account to continue.'}</p>
