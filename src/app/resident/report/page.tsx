...
export default function ReportIncidentPage() {
  ...
  if (step === 'review') return (
    <main className="min-h-screen bg-[#f5f8fc] px-4 py-5 sm:px-6"><div className="mx-auto max-w-xl">
      ...
      <section className="mt-6 space-y-3 rounded-3xl border border-[#dce6f0] bg-white bc-panel p-5 shadow-sm">
  ...
  return (
    <main className="min-h-screen bg-[#f5f8fc] px-4 py-5 sm:px-6"><div className="mx-auto max-w-xl">
      ...
      <form onSubmit={handleReview} className="mt-5 space-y-4">
        <section className="rounded-3xl border border-[#dce6f0] bg-white bc-panel p-5 shadow-sm"><div className="flex items-center justify-between"><label htmlFor="description" className="text-sm font-extrabold">What happened?</label><span className="rounded-full bg-blue-50 px-2.5 py-1 text-[11px] font-bold text-[#0b66c3]">AI assisted</span></div><textarea id="description" required minLength={5} maxLength={4000} value={description} onChange={(e) => setDescription(e.target.value)} rows={6} placeholder="Example: May aksidente sa highway malapit sa palengke. Mukhang may nasaktan." className="mt-3 w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-6 outline-none transition placeholder:text-slate-400 focus:border-[#0b66c3] focus:bg-white focus:ring-4 focus:ring-blue-50" /><div className="mt-2 flex justify-between text-[11px] text-slate-500"><span>Write naturally in Filipino, English, or Taglish.</span><span>{description.length}/4000</span></div></section>
        <section className="rounded-3xl border border-[#dce6f0] bg-white bc-panel p-5 shadow-sm"><label htmlFor="category" className="text-sm font-extrabold">Incident category <span className="font-normal text-slate-400">(optional)</span></label><select id="category" value={category} onChange={(e) => setCategory(e.target.value)} className="mt-3 w-full rounded-2xl border border-slate-200 bg-white bc-panel px-4 py-3 text-sm outline-none focus:border-[#0b66c3]"><option value="">Let BConnect classify it</option>{categories.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></section>
        <section className="rounded-3xl border border-[#dce6f0] bg-white bc-panel p-5 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-extrabold">Incident location</p><p className="mt-1 text-xs leading-5 text-slate-500">Use GPS when possible. Only share your location when safe.</p></div><button type="button" onClick={captureLocation} disabled={locationState === 'loading'} className="shrink-0 rounded-xl bg-blue-50 px-3 py-2 text-xs font-bold text-[#0b66c3] disabled:opacity-50">{locationState === 'loading' ? 'Locating…' : locationState === 'ready' ? 'Refresh GPS' : 'Use GPS'}</button></div><div className={`mt-3 rounded-2xl p-4 text-xs font-medium ${locationState === 'ready' ? 'bg-emerald-50 text-emerald-800' : locationState === 'error' ? 'bg-amber-50 text-amber-800' : 'bg-slate-50 text-slate-600'}`}>{locationMessage}</div></section>
